...
export default function BankAccountsList({ accounts, narrationHint }: { accounts: BankAccount[]; narrationHint?: string }) {
  ...
  return (
    <div>
      <p className="mb-3 text-sm font-semibold uppercase tracking-wide2 text-ink-muted">
        Bank transfer
      </p>
        ...
        {accounts.map((a, i) => (
          ...
          >
            ...
            <button
              ...
              className="rounded-full border border-ink/15 px-3 py-1.5 text-sm font-semibold text-ink transition hover:border-red hover:text-red"
            >
